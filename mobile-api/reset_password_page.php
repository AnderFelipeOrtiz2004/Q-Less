<?php
/**
 * Formulario web para cambiar contraseña (enlace desde correo).
 * GET: muestra formulario estilo login. POST: aplica el cambio.
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($password !== $confirm) {
        ...
        $result = perform_password_reset($conn, $email, $code, $password);
        if ($result['ok']) {
            $success = true;
...
<head>
  ...
  <title>Q-LESS — Cambiar contraseña</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    input:focus { outline: none; border-color: #3EC13B; border-width: 2px; padding: 13px 15px; }
...
<body>
  <div class="card">
    <div class="header"><h1>Q-LESS</h1><p style="margin-top:8px;opacity:.9;">Cambiar contraseña</p></div>
    <div class="content">
      <?php if ($success): ?>
        <div class="ok"><?= htmlspecialchars($message, ENT_QUOTES, 'UTF-8') ?></div>
        <p class="hint">Ya puedes cerrar esta página e iniciar sesión en la aplicación con tu nueva contraseña.</p>
        <?php if ($appUrl !== ''): ?>
          <a class="btn-outline" href="<?= htmlspecialchars($appUrl, ENT_QUOTES, 'UTF-8') ?>">Abrir aplicación Q-LESS</a>
        <?php endif; ?>
      <?php else: ?>
        <?php if ($error !== ''): ?><div class="err"><?= htmlspecialchars($error, ENT_QUOTES, 'UTF-8') ?></div><?php endif; ?>
        <p class="hint">Ingresa el código de 6 dígitos que recibiste por correo y tu nueva contraseña.</p>
        <form method="post" action="">
          ...
          <input type="text" id="code" name="code" value="<?= htmlspecialchars($code, ENT_QUOTES, 'UTF-8') ?>" required maxlength="6" inputmode="numeric" autocomplete="one-time-code">
