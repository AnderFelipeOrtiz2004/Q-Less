...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    ...
    _purchasesEnabled = widget.purchasesEnabled || _isAdmin;
    _loadProducts();
    _loadCartFromServer();
    _loadFavoriteIds();
    _loadPurchasesStatus();
    _startBannerTimer();
    _startStockSyncTimer();
  }

  Future<void> _loadPurchasesStatus() async {
    if (_uid <= 0 || _isAdmin) return;
    try {
      final user = await UserService.fetchUser(userId: _uid);
      if (!mounted) return;
      setState(() {
  ...
  void _startStockSyncTimer() {
    ...
    _stockSyncTimer = Timer.periodic(const Duration(seconds: 30), (_) {
      if (!mounted) return;
  ...
  Future<void> _loadCartFromServer() async {
    ...
    try {
      ...
      for (final raw in list) {
        ...
        final stockLeft = int.tryParse(raw['stock']?.toString() ?? '') ?? 0;
        final availableStock =
            int.tryParse(raw['available_stock']?.toString() ?? '') ??
                (stockLeft + qty);
  ...
  void _applyProductUpdateQuietly(List<Product> loadedProducts) {
    final previousIds = products.map((p) => p.id).toSet();
    final nextIds = loadedProducts.map((p) => p.id).toSet();
    var changed = previousIds.length != nextIds.length ||
    ...
    if (!changed) {
      ...
      for (final product in products) {
        ...
        if (fresh.availableStock != product.availableStock ||
            fresh.stock != product.stock ||
            fresh.precio != product.precio ||
            fresh.nombre != product.nombre ||
            fresh.categoria != product.categoria) {
          changed = true;
          break;
        }
      }
