...
class _ProductsPageState extends State<ProductsPage> {
  ...
  Future<void> _loadProducts({bool silent = false}) async {
    ...
    try {
      ...
      for (final product in products) {
        ...
        if (rawCategory.isEmpty || rawCategory.toLowerCase() == 'todos') {
          continue;
        }
      ...
      final mergedCategories = <String>{'Todos', ...categories.skip(1)};
      mergedCategories.addAll(normalizedCategoryMap.values);
  ...
  Widget _buildBody() {
    ...
    final productosFiltrados = selectedCategory == 'Todos'
        ...
        : listaProductosCompleta
            .where(
              (producto) => productMatchesCategory(
                producto.categoria,
                selectedCategory,
              ),
            )
